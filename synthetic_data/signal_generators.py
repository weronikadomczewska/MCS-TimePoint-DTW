...
class Mader2014Model:
    ...
    def step(self, p_mean, dt):
        ...
        acc = -self.a * self.v_dyn - self.b * self.dv_dyn_dt + self.c * ca_curve + self.M * dp_dt
...
class CBFVSignalGenerator:
    def __init__(self, fs=100, duration=10, hr_range=None, noise_std=None, ca_params=None, abp_params=None):
        self.fs = fs
        self.duration = duration
        ...
        self.timeline = TimelineGenerator(duration=duration, min_hr=self.hr_range[0], max_hr=self.hr_range[1])
    ...
    def asymmetric_pulse(self, t_local, mfv_beat):
        ...
        wave = (1.0 / t_shifted) * np.exp(- (np.log(t_shifted) - (-1.0))**2 / (2 * 0.4**2))
    ...
    def generate(self):
        ...
        # Szum pomiarowy
        abp_noise, cbfv_noise = self.noise_std if self.noise_std else (np.random.uniform(1.0, 3.0), np.random.uniform(1.5, 4.0))
...
def plot_signals(
    ...
    if abp_kp is not None:
        scatter_abp = ax1.scatter(
            t[abp_kp], abp[abp_kp],
            color="darkred", s=25, label="ABP keypoints"
        )
    ...
    if cbfv_kp is not None:
        scatter_cbfv = ax2.scatter(
            t[cbfv_kp], cbfv[cbfv_kp],
            color="navy", s=25, label="CBFV keypoints"
        )
...
def plot_signals_stacked(t, abp, cbfv, abp_kp=None, cbfv_kp=None,
    ...
    if abp_kp is not None:
        ax1.scatter(t[abp_kp], abp[abp_kp],
                    color="darkred", s=25, label="ABP keypoints")
    ...
    if cbfv_kp is not None:
        ax2.scatter(t[cbfv_kp], cbfv[cbfv_kp],
                    color="navy", s=25, label="CBFV keypoints")
...
def generate_dataset(n_samples=5000, save_dir="synthetic_dataset", plot_dir="plots", fs=100, duration=10):
